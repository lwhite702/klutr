...
export function useSectionSummary(sectionId: string, defaultExpanded = true) {
  ...
  const toggle = useCallback(() => setAndPersist((prev) => !prev), [setAndPersist])
...
export function useSectionTour(sectionId: string, steps: TourStep[], options?: SectionTourOptions) {
